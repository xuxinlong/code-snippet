(function () {
    ...
    // 数字格式化 过滤器
    etpl.addFilter('slice', function (value, num) {
        return value.length > num ? value.slice(0, num - 1) + '...' : value;
    });
    // 数字格式化 过滤器
    ...
    // 数字格式化 过滤器
    etpl.addFilter('fixed', function (value, fixedNum) {
        return Number(value).toFixed(fixedNum);
    });
    // 去掉小数后面补位的0 过滤器
    etpl.addFilter('number', function (value) {
        if (value.indexOf('-') > -1) {
            value = value.split('-');
        }
        return value instanceof Array ? Number(value[0]) + '-' + Number(value[1]) : Number(value);
    });
    ...
    ecui.esr.onready = function () {
        ...
        // 设置 默认路由
        ecui.esr.DEFAULT_PAGE = '/preview/pc-page/pc-example';
